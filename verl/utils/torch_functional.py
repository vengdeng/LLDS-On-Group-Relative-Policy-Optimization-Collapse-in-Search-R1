...
def masked_min(values, mask, axis=None):
    """Compute min of tensor with a masked values that is non zero 0."""
    masked_values = values * mask
    masked_values = torch.where(masked_values > 0, masked_values, torch.tensor(float('inf')))
    # Take the min along the last axis
    min_values, _ = torch.min(masked_values, dim=-1)
