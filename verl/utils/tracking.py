...
class Tracking(object):
    ...
    def __init__(
        ...
        if "tracking" in default_backend or "wandb" in default_backend:
            ...
            # Monkey-patch wandb.util.working_set to handle packages with None metadata
            import wandb.util

            def safe_working_set():
                """Generator that yields package info, skipping packages with None metadata"""
                for d in distributions():
                    try:
                        if d.metadata is not None and "Name" in d.metadata:
                            yield wandb.util.InstalledDistribution(
                                key=d.metadata["Name"], version=d.version
                            )
                    except (TypeError, AttributeError, KeyError):
                        # Skip packages with invalid or None metadata
                        continue
